...
// tb1qqgfgglqfw9j8vlv2vvp8l220sm8wau64rrm974【软件的】
// bc1qqgfgglqfw9j8vlv2vvp8l220sm8wau64f9qk9x【软件的】
// 1BxY1CFKWd35MTCpLfqAjoFDt5MQQZ4ai【实际的】
// 本地和软件私钥不同的原因
// 比特币的设计中，同一个私钥可以对应多个地址，这是由比特币的地址生成机制决定的。比特币使用的是椭圆曲线加密算法（ECDSA-secp256k1），私钥是一个256位的随机数。通过私钥，可以计算出对应的公钥。然后，将公钥经过多次哈希散列及处理，生成比特币地址。
// 比特币地址的生成过程包括以下步骤：
// 将完整公钥（头部标识 X-integer Y-integer）进行一次SHA256运算，得到结果A。
// 对结果A进行一次RIPEMD160运算，得到结果B，B的长度为20bytes。
// 在结果B前加上网络ID字节（例如，比特币主网的ID为0x00），得到结果C，C的长度为21bytes。
// 使用Base58编码对结果C进行编码，得到最终的比特币地址。
// 在这个过程中，公钥经过哈希散列处理，生成了多个地址。这些地址都是基于同一个私钥的，因此它们都共享同一个私钥。不同的地址之间只是哈希散列的结果不同，但它们背后的私钥是相同的。这种设计允许用户在一次交易中生成多个地址，提高交易的隐私性。
